...
namespace SchulungKK.Pages
{
    public class AnmeldenModel : PageModel
    {
        ...
        [BindProperty] public string LoginName { get; set; } = string.Empty;
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var benutzer = await _context.Benutzer.FirstOrDefaultAsync(b => b.Aktiv && (b.Benutzername.ToLower() == normalisierteEingabe || b.Email.ToLower() == normalisierteEingabe));

            // ✅ BCrypt.Verify prüft das Passwort gegen den gespeicherten Hash
            if (benutzer == null || !BCrypt.Net.BCrypt.Verify(Password, benutzer.Passwort))
            {
